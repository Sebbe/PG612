...
Model::Model(std::string filename, bool invert) {
	...
	loadRecursive(root, invert, vertexNormal_data, scene, scene->mRootNode);
...
void Model::loadRecursive(MeshPart& part, bool invert,
	...
	for (unsigned int n=0; n < node->mNumMeshes; ++n) {
		...
		part.first = vertexNormal_data.size()/3;
	...
	for (unsigned int n = 0; n < node->mNumChildren; ++n) {
		...
		loadRecursive(part.children.back(), invert, vertexNormal_data, scene, node->mChildren[n]);
	}
}